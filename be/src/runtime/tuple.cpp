...
void Tuple::materialize_exprs(TupleRow* row, const TupleDescriptor& desc,
    ...
    for (int i = 0; i < slots.size(); ++i) {
        ...
        if (slot_type == PrimitiveType::TYPE_CHAR || slot_type == PrimitiveType::TYPE_VARCHAR ||
            slot_type == PrimitiveType::TYPE_HLL || slot_type == PrimitiveType::TYPE_STRING) {
            DCHECK(expr_type == PrimitiveType::TYPE_CHAR ||
                   expr_type == PrimitiveType::TYPE_VARCHAR ||
                   expr_type == PrimitiveType::TYPE_HLL || expr_type == PrimitiveType::TYPE_STRING);
        } else if (slot_type == PrimitiveType::TYPE_DATE ||
                   slot_type == PrimitiveType::TYPE_DATETIME) {
            DCHECK(expr_type == PrimitiveType::TYPE_DATE ||
                   expr_type == PrimitiveType::TYPE_DATETIME);
        } else if (slot_type == PrimitiveType::TYPE_ARRAY) {
            DCHECK(expr_type == PrimitiveType::TYPE_ARRAY);
        } else {
            DCHECK(slot_type == PrimitiveType::TYPE_NULL || slot_type == expr_type);
        }
