    ...
    if (_is_all_column_basic_type) {
        ...
        for (int i = 0; i < _schema.num_column_ids(); i++) {
            ...
            if (pred_set.find(cid) != pred_set.end()) {
                ...
                _first_read_column_ids.push_back(cid);
                _is_pred_column[cid] =
                        true; // in this case, non-predicate column should also be filtered by sel idx, so we regard it as pred columns
            }
        }
...
Status SegmentIterator::next_batch(vectorized::Block* block) {
    ...
    if (_vec_pred_column_ids.empty() && _short_cir_pred_column_ids.empty()) {
        ...
        if (_is_all_column_basic_type) {
            _output_column_by_sel_idx(block, _first_read_column_ids, sel_rowid_idx, selected_size,
                                      is_mem_reuse);
            return Status::OK();
        }
        ...
        _output_column_by_sel_idx(block, _short_cir_pred_column_ids, sel_rowid_idx, selected_size,
                                  is_mem_reuse);
        // 4.3 output vectorizatioin predicate column
        _output_column_by_sel_idx(block, _vec_pred_column_ids, sel_rowid_idx, selected_size,
                                  is_mem_reuse);
    }
