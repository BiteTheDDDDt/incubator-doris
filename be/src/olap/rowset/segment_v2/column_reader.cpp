...
void DefaultValueColumnIterator::insert_default_data(vectorized::MutableColumnPtr &dst, size_t n) {
    ...
    auto insert_n_data = [data_ptr, data_len, n, &dst]() {
        for (size_t i = 0; i < n; ++i) {
            dst->insert_data(data_ptr, data_len);
        }
    };

    if (type == OLAP_FIELD_TYPE_OBJECT || type == OLAP_FIELD_TYPE_HLL) {
        if (type == OLAP_FIELD_TYPE_OBJECT) {
        ...
        data_ptr = (char *) &int128;
        data_len = sizeof(int128);
